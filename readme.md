...
1. **Helmet Security Protection**
   - The API is protected using Helmet, a middleware that helps secure the app by setting various HTTP headers, including:
     - **Strict-Transport-Security (HSTS):** Ensures secure connections by requiring HTTPS.
     - **X-XSS-Protection:** Enables cross-site scripting (XSS) filtering in the browser.
     - **X-Content-Type-Options (No Sniff):** Prevents browsers from interpreting files as something else (e.g., JavaScript files being treated as text).
...
- **Request Example**:
```bash
curl http://localhost:3000/incidents/680c64496c3961371a012edb
```
- **Response**:
```bash
{
...
- **Request Example**: 
```bash
curl -X POST http://localhost:3000/incidents -H "Content-Type: application/json" -d "{\"title\":\"Example Incident\",\"description\":\"Something happened with an AI system.\",\"severity\":\"High\"}"
...
- **Request Example**:
```bash
curl -X DELETE http://localhost:3000/incidents/6816fbac4e0ac011643e2e57
```
- **Response**:
```json
{
